...
namespace BinarySearch
{
    public abstract class BinarySearchTree<TItem> : IEnumerable<TItem> where TItem : IComparable<TItem>
    {
        ...
        public BinarySearchTree(IComparer<TItem> comparer)
        {
            this.comparer = (comparer == null) ? Comparer<TItem>.Default : comparer;
        }

        public BinarySearchTree(IEnumerable<TItem> collection) : this(collection, null) { }

        public BinarySearchTree(IEnumerable<TItem> collection, IComparer<TItem> comparer) : this(comparer)
        {
            AddRange(collection);
        }
        ...
        {
            if (IsEmpty)
                throw new InvalidOperationException("Пустое дерево.");
