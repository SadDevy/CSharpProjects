...
namespace BinarySearch
{
    public class RecursiveTree<TItem> : BinarySearchTree<TItem> where TItem : IComparable<TItem>
    {
        ...
        private IEnumerable<TItem> GetArroundInOrder(Node<TItem> root)
        {
            if (root == null)
                yield break;
            ...
            if (root.Left != null)
            {
                foreach (TItem item in GetArroundInOrder(root.Left))
                    yield return item;
            }
            ...
            if (root.Right != null)
            {
                ...
                foreach (TItem item in GetArroundInOrder(root.Right))
                    yield return item;
            }
        }
        ...
        {
            return GetArroundInOrder(Root).GetEnumerator();
        }
